    ...
    static showModalWriteGoal(interaction){
        if(interaction.customId.includes('writeGoal')){
			...
			.addComponents(
				...
				new TextInputComponent().setCustomId('goal').setLabel("Goal (that excites you & can be quantify)").setPlaceholder("e.g. 10 design & get 1 client / build todo app & get 1000 users / 20 content & get 100K impressions").setStyle("LONG").setRequired(true),
        ...
        if(interaction.customId.includes('editGoal')){
			...
			.addComponents(
				...
				new TextInputComponent().setCustomId('goal').setLabel("Goal (that excites you & can be quantify)").setDefaultValue(goal).setPlaceholder("e.g. 10 design & get 1 client / build todo app & get 1000 users / 20 content & get 100K impressions").setStyle("LONG").setRequired(true),
